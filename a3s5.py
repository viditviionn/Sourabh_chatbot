...
documents = load_pdf(r'C:\Users\satyam\OneDrive\Desktop\flask\A3S.pdf')
...
def generate_response(query):
    ...
    # Print the selected chunks
    for result in retriever_results:
        print(f"Selected Chunk: {result['text'][:100]}...\nScore: {result['score']:.4f}")
    print("-" * 50)

    # Filter out results with low similarity scores (indicating they are not relevant)
    filtered_results = [result for result in retriever_results if result['score'] < 50]
    ...
    # Add the filtered result count to the response
    responses.append(f"\nTotal Filtered Results: {filtered_count}\n{'='*50}")
